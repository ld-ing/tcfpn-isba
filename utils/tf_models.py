...
# Li Ding
# Mar. 2018
# Modified from: https://github.com/colincsl/TemporalConvolutionalNetworks/blob/master/code/tf_models.py
...
def Dilated_TCN(num_feat, num_classes, nb_filters, dilation_depth, nb_stacks, max_len,
    ...
    def residual_block(x, s, i, activation):
        ...
        if causal:
            ...
            conv = AtrousConv1D(nb_filters, 2, atrous_rate=2 ** i, padding='same',
                                name='dilated_conv_%d_tanh_s%d' % (2 ** i, s))(x)
            conv = Cropping1D((0, (2 ** i) // 2))(conv)
        else:
            conv = AtrousConv1D(nb_filters, 3, atrous_rate=2 ** i, padding='same',
                                name='dilated_conv_%d_tanh_s%d' % (2 ** i, s))(x)
